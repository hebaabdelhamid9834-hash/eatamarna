            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 20),
                    child: PaymentTextRow(title: S.of(context).tripDate, value: bookingResponse.bookingDetails?.date??''),
                  ),
                  SizedBox(height: 18),
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 20),
                    child: PaymentTextRow(title: S.of(context).tripTime, value: bookingResponse.bookingDetails?.time.toString()??''),
                  ),
                  SizedBox(height: 18),
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 20),
                    child: PaymentTextRow(title: S.of(context).currencyLabel, value: bookingResponse.bookingDetails?.currency.toString()??''),
                  ),
        SizedBox(height: 18),
        Padding(
        padding: const EdgeInsets.symmetric(horizontal: 20),
        child: PaymentTextRow(title: S.of(context).subscriptionPrice, value:bookingResponse.bookingDetails?.totalPrice.toString()??''),
        ),
                  SizedBox(height: 20),
                  Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              Text(
                                bookingResponse.bookingDetails?.totalPrice.toString()??'',
                                style: TextStyle(
                                  fontSize: 19,
                                  color: ColorManager.black,
                                  fontWeight: FontWeight.w700,
                              ...
                              SizedBox(width: 3,),
                              Text(
                                bookingResponse.bookingDetails?.currency.toString()??'',
                                style: TextStyle(
                                  fontSize: 16,
                                  color: ColorManager.black,
                        ...
                        return Expanded(
                          child: GestureDetector(
                            onTap: () {
                              setState(() {
                                selectedIndex = index;
                                bloc.add(ChangePaymentMethod(selectedItem: index));
                              });
                            },
                            child: Container(
                              ...
                              child: Column(
                                children: [
                                  ...
                                  Text(
                                    payments[index]["key"] == "cashOnDelivery" ? S.of(context).cashOnDelivery : S.of(context).knet,
                                    style: TextStyle(
                                      fontWeight: FontWeight.w700,
                                      fontSize: 12,
                                      color: isSelected
          ...
          return Center(child: Text(state.error,style: TextStyle(color: ColorManager.red),));
        }
        else if (state is LoadingGetBookingDetailsState){
          return Center(child: CircularProgressIndicator(color: ColorManager.lightBlue,));
        }
        else{
          return SizedBox();
        }
